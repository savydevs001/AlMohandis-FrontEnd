...
const GroupCard = ({ group }: { group: any }) => {
  ...
  const firstSchedule = firstSubject.schedules?.[0] || {};

  const formattedDays = firstSchedule.days?.join(", ") || "N/A";
  const formattedTime = firstSchedule.startTime
    ? new Date(firstSchedule.startTime).toLocaleString()
    : "N/A";
      ...
      <div className="flex flex-col items-center gap-4 lg:flex-row">
        ...
        <div className="space-y-1">
          ...
          <p className="text-lg font-semibold text-pTag">
            Duration:{" "}
            <span className="font-normal">{`${formattedDays} at ${formattedTime}`}</span>
          </p>
          <p className="text-sm font-semibold text-pTag">
            Teacher: <span className="font-normal">{firstSubject.teacherName || "N/A"}</span>
          </p>
        </div>
      </div>
      <div className="flex flex-col lg:w-[25%] w-[100%] items-start gap-2 lg:border-l-4 border-BgColor px-4">
        <NavLink to={`group/${group.groupId}`} className="w-full">
